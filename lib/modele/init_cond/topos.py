...
SheetInfo = collections.named_tuple('SheetInfo', ('name', 'elevI'))
...
class ConcatCoo(object):
    ...
    def add(A):
        ...
        self.row.append(iter(A.row))
        self.col.append(iter(A.col))
        self.data.append(iter(A.data))

    def __call__(self):
        return scipy.sparse.coo_matrix((A.data, (A.row, A.col)), self.shape)
...
class Topos(object):
    def __init__(self, icebin_in, topo_in)
        ...
        with netCDF4.Dataset(icebin_in) as nc:
            ...
            self.ec_base = self.sealand_base + nsealand
    ...
    def get_fractions(sheets):
        ...
        AvE_c = ConcatCoo()
        elevE = np.zeros((self.nE_gcm,)) + np.nan
        ...
        for sheet in sheets:
            ...
            elevE_i = icebin.coo_multiply(EvI, sheet.elevI)
            concat_vector(elevE, elevE_i)

            elevA_i = icebin.coo_multiply(AvI, sheet.elevI)
            concat_vector(elevA, elevA_i)

        AvE = AvE_c()

        # ------------------ Read original surface area fractions
        with netCDF4.Dataset(iTOPO) as nc:
        ...
        fhc[self.legacy_base,fgice != 0] = 1.   # Legacy ice for Greenland and Antarctica
        underice[self.legacy_base,fgice != 0] = UI_NOTHING

        elevE[self.legacy_base,:] = zatmo_m
        # Assume elevation=0 for non-ice-sheet areas.  This is OK for ocean portions,
        # not OK for bare land portions.
        elevE[self.legacy_base,_maskA] = zatmo_m[_maskA]
        ...
        fhc[self.sealand_base,_maskA] = 0.
        underice[self.sealand_base,_maskA] = 0
        elevE[self.sealand_base,_maskA] = 0.
        ...
        fhc[self.sealand_base+1,_maskA] = 1.
        underice[self.sealand_base+1,_maskA] = UI_NOTHING
        elevE[self.sealand_base+1,_maskA] = elevA[_maskA]

        # ---------- Segment 2: Full Elevation Classes
        shapeE2 = (self.nhc_ice, self.nA)
        shapeE2_gcm = (nhc_gcm, self.nA)
        ...
        for iA,iE,weight in zip(AvE.row, AvE.col, AvE.data):
            ...
            fhcE[self.ec_base+ihc,iA] = weight
            undericeE2[self.ec_base+ihc,iA] = UI_ICEBIN

        for i in range(0,self.nA):
            elevE[self.ec_base:,i] = hcdefs_ice[:]
